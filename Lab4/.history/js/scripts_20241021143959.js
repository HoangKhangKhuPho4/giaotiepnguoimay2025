...
document.addEventListener("DOMContentLoaded", function () {
  ...
  yourProgressLink.addEventListener("click", function (e) {
    e.preventDefault();
    progressDropdown.style.display =
      progressDropdown.style.display === "block" ? "none" : "block";
...
document.addEventListener("DOMContentLoaded", function () {
  // Thêm sự kiện cho liên kết "5 more"
  const showMoreLink = document.getElementById("show-more-languages");
  ...
  showMoreLink.addEventListener("click", function (event) {
    ...
    if (moreLanguages.style.display === "none") {
      moreLanguages.style.display = "block"; // Hiển thị danh sách ngôn ngữ
    } else {
      moreLanguages.style.display = "none"; // Ẩn nếu đã hiển thị
    }
  });
});
